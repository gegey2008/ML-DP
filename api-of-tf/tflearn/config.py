...
#Created Time: 2017-10-29 11:37:02
############################
...
def init_graph(seed=None, log_device=False, num_cores=0, gpu_memory_fraction=0,
    ...
    Initialize a graph with sepcific parameters.

    Arguments:
        seed: `int`. set the graph random seed.
        log_device: `bool`. Log device palcement or not.
        ...
        soft_palcement: `bool`. Wether soft plcaement is allowed. If true,
...
def is_training(is_training=False, session=None):
    ...
    Examples:
        ...
        >> training_mode = tf.get_training_mode()
    ...
    init_training_mode()
    if is_training:
        tf.get_collection('is_training_ops')[0].eval(session)
    else:
        tf.get_collection('is_training_ops')[1].eval(session)


def get_training_mode():
    """ get_training_mode.

    Retruns variable in-use to set training mode.
...
def init_training_mode():
    ...
    """
    # 'is_training' collection stores the training mode varible.
    coll = tf.get_collection('is_training')
    if len(coll) == 0:
        tr_var = variable("is_training", dtype=tf.bool, shape=[],
